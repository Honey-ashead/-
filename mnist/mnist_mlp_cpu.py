...
class MNIST_MLP(object):
    def __init__(self, batch_size=100, input_size=784, hidden1=32, hidden2=16, hidden3=8, out_classes=10, lr=0.01,
                 max_epoch=2,
                 print_iter=100):
    ...
    def save_model(self, param_dir):
        ...
        params = {'w1': (self.fc1.save_param())[0], 'b1': (self.fc1.save_param())[1], 'w2': (self.fc2.save_param())[0],
                  'b2': (self.fc2.save_param())[1], 'w3': (self.fc3.save_param())[0], 'b3': (self.fc3.save_param())[1],
                  'w4': (self.fc4.save_param())[0], 'b4': (self.fc4.save_param())[1]}
    ...
    def backward(self):  # 神经网络的反向传播
        ...
        dh3 = self.fc4.backward(dloss)
        dh3 = self.relu3.backward(dh3)
...
def build_mnist_mlp(param_dir='weight.npy'):
    h1, h2, h3, e = 64, 32, 16, 20
    mlp = MNIST_MLP(hidden1=h1, hidden2=h2, hidden3=h3, max_epoch=e)
    mlp.load_data()
    mlp.build_model()
    mlp.init_model()
    mlp.train()
    mlp.save_model('mlp-%d-%d-%d-%d epoch.npy' % (h1, h2, h3, e))
